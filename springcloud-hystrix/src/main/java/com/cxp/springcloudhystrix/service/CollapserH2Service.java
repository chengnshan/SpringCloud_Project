 ...
 */
@Service
public class CollapserH2Service {

    @Autowired
    private RestTemplate restTemplate;

    public UserInfo_H2Database getUserInfoById(Integer id){
        UserInfo_H2Database userInfo = restTemplate.getForObject(
                "http://springcloud-producer-h2database/producer-h2database/collapserById/{1}",
                UserInfo_H2Database.class, id);
        return userInfo;
    }

    public List<UserInfo_H2Database> batchUserInfos(List<Integer> ids){
        System.out.println("batchUserInfos----"+ids+"Thread.currentThread().getName():" +
                Thread.currentThread().getName());
        UserInfo_H2Database[] userInfos = restTemplate.getForObject(
                "http://springcloud-producer-h2database/producer-h2database/collapserMethod?ids={1}",
                UserInfo_H2Database[].class, StringUtils.join(ids, ","));
    ...
    public Future<UserInfo_H2Database> annoGetUserInfoById(Integer id){
        System.out.println("执行单个获取的方法  ==  "+ id);
        return null;
    }

    @HystrixCommand
    public List<UserInfo_H2Database> annoBatchUserInfos(List<Integer> ids){
